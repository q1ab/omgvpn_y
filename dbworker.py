...
class User:
    ...
    async def NewPay(self,bill_id,summ,time_to_add,mesid,url_topay):
        pay_info = await self.PaymentInfo()
        #print(pay_info)
        if pay_info is None:
            db = await aiosqlite.connect(DBCONNECT)
            await db.execute(f"INSERT INTO payments (tgid,bill_id,amount,time_to_add,mesid,url_topay) values (?,?,?,?,?,?)",
                             (self.tgid, str(bill_id),summ,int(time_to_add),str(mesid),str(url_topay)))
            await db.commit()
