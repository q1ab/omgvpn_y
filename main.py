...
async def Work_with_Message(m: types.Message):
    ...
    if m.from_user.id == CONFIG["admin_tg_id"]:
        ...
        if e.demojize(m.text) == "Всех пользователей":
            ...
            await bot.send_message(m.from_user.id, e.emojize(readymes), reply_markup=await buttons.admin_buttons(),
                                   parse_mode="HTML")
            return
    ...
    if e.demojize(m.text) == "Продлить :money_bag:":
        ...
        if not payment_info is None:
            urltopay = str(payment_info['url_topay'])
    ...
    if e.demojize(m.text) == "Как подключить :gear:":
        if user_dat.trial_subscription == False:
            ...
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("Инструкция для iPhone"),
                                           url="https://telegra.ph/Gajd-na-ustanovku-WireGuard-01-16"))
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("Инструкция для Android"),
                                           url="https://telegra.ph/Gajd-na-ustanovku-WireGuard-Android-01-16"))
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("Проверить VPN"),
                                           url="https://2ip.ru/"))
            config = open(f'/root/wg0-client-{str(user_dat.tgid)}.conf', 'rb')
            await bot.send_document(chat_id=m.chat.id, document=config,
                                    visible_file_name=f"OMGVPN_{datetime.now().date().strftime('%d%m%y')}.conf",
...
async def Cancel_payment(call: types.CallbackQuery):
    ...
    if not payment_info is None:
        ...
        await bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.id, text="Платеж отменен!",
                                    reply_markup=None)
...
async def Buy_month(call: types.CallbackQuery):
    ...
    if payment_info is None:
        ...
        await bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.id,
                                    text=f"<b>Оплата: VPN на {str(Month_count)} мес.\n\nСумма оплаты: <code>{str(Month_count * CONFIG['one_month_cost'])} ₽</code></b>\nОплатите счет в течение 45 минут!",
                                    parse_mode="HTML", reply_markup=Butt_payment)
...
async def DeleteUserYesOrNo(call: types.CallbackQuery):
    ...
    if "DELETYES:" in call.data:
        ...
        subprocess.call(f'./deleteuserfromvpn.sh {str(idstatic)}', shell=True)
            ...
            if len(log) > 0:
                ...
                for i in log:
                    if client.operation_history(label=f'Оплата-VPN-на-1-мес.-для-пользователя-{i["tgid"]}-{i["bill_id"]}').operations != []:
                        ...
                        Butt_main = types.ReplyKeyboardMarkup(resize_keyboard=True)
                        dateto = datetime.utcfromtimestamp(int(passdat) + CONFIG['UTC_time'] * 3600).strftime(
                            '%d.%m.%Y %H:%M')
                        timenow = int(time.time())
                        if int(passdat) >= timenow:
                            Butt_main.add(
                        ...
                        BotChecking.edit_message_reply_markup(chat_id=i['tgid'], message_id=i['mesid'],
                                                              reply_markup=None)
                        BotChecking.send_message(i['tgid'],
